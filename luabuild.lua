...
local function sync()
	...
	while #threads > 0 do
		for i=1, #threads do
			if (threads[i].status ~= "running") then
				if not threads[i][1] then
					errors[#errors+1] = threads[i][2]
				end
...
reflect.CC_STATE = CC_STATE

local run_t = lanes.gen("*", run)

local function compile(file)
	local compiler_command = CC_STATE.compiler .. " -fdiagnostics-color=always "
...
local function link(target, files)
	...
	for i=1, #CC_STATE.libs do
		compiler_command = compiler_command .. "-l"..CC_STATE.libs[i].." "
	end
